...
const sendMyMessage = (chatWidowId, fromUser, message) => {
    ...
    let image = loggedInUser.user_id == fromUser.user_id ? fromUser.user_image : 'another_user.jpg';
...
const createRoom = (id) => {
    ...
    let data = { room: room, userId: loggedInUser.userId, withUserId: id }
    socket.emit('create', data);
    openChatWindow(room);
}
socket.on('updateUserList', function (userList) {
    let loggedInUser = JSON.parse(sessionStorage.getItem('user'));
    $('#user-list').html('<h4>ONLINE USERS</h4><ul></ul>');
